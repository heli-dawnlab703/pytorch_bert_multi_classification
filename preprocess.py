...
class Processor:
    ...
    def get_examples(self, raw_examples, set_type,id2label):
        examples = []
        # 这里是从json数据中的字典中获取
        self.convert_data_txt(examples, raw_examples, set_type,id2label)
        return examples

    def convert_data_json(self, examples, raw_examples, set_type):
    ...
    def convert_data_txt(self, examples, raw_examples, set_type,id2label):
        for line in raw_examples.split('\n'):
            labels = []
            split_result = re.split(r'\t', line)
            text = split_result[0]
            labels.append(id2label.get(int(split_result[1])))
            examples.append(InputExample(set_type=set_type,
                                         text=text,
                                         labels=labels))
...
def get_out(processor, json_path, args, label2id, mode):
    ...
    examples = processor.get_examples(raw_examples, mode, id2label)
    for i, example in enumerate(examples):
        print(example.text)
...
if __name__ == '__main__':
    ...
    args.bert_dir = './model_hub/bert_base_chinese/'
